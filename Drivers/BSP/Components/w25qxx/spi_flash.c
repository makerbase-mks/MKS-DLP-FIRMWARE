...
u16 W25QXX_ReadID(void)
{
	u16 Temp = 0;	  
	W25QXX_CS=0;				    
	SPIn_ReadWriteByte(0x90);//发送读取ID命令	    
	SPIn_ReadWriteByte(0x00); 	    
	SPIn_ReadWriteByte(0x00); 	    
	SPIn_ReadWriteByte(0x00); 	 			   
	Temp|=SPIn_ReadWriteByte(0xFF)<<8;  
	Temp|=SPIn_ReadWriteByte(0xFF);	 
	W25QXX_CS=1;				    
	return Temp;
}   		    
...
void SPI_FLASH_BufferRead(u8* pBuffer, u32 ReadAddr, u16 NumByteToRead)
{
  ...
  {
      HAL_SPI_Receive_DMA(&hspin, pBuffer, NumByteToRead);      //启动DMA接收
      while(hdma_spin_rx.State ==HAL_DMA_STATE_BUSY);          //等待DMA接收完成
...
u16 SPI_FLASH_SendHalfWord(u16 HalfWord)
{
  ...
  /* Send Half Word through the SPIn peripheral */
  SPI_I2S_SendData(SPIn, HalfWord);
