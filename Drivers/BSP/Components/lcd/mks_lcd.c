...
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef_TFT* GPIO_InitStruct)
{
...
{ 	
  ...
  GPIO_Init(GPIOG, &GPIO_InitStructure);//初始化 //PB15 推挽输出,控制背光
	LCD_LED = 1;
...
GPIO_Init(GPIOG, &GPIO_InitStructure);//初始化 
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12|GPIO_Pin_7;//PD12,FSMC_A17	//PD7,FSMC_NE1
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用输出
  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
...
#if defined(USE_STM32F407ZET)
#if MKS_BOARD_TYPE  == 0
  GPIO_PinAFConfig(GPIOF,GPIO_PinSource0,GPIO_AF_FSMC);//PF12,AF12
#else
...
void LCD_TEST()
{
  /*
		...
		LCD_ShowString(30,90,200,16,16,"ATOM@ALIENTEK");
 		//LCD_ShowString(30,110,200,16,16,lcddev.id);		//显示LCD ID	      					 
