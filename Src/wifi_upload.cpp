...
void ResetWiFiForUpload(int begin_or_end)
{
	...
	GPIO_InitStructure.Pin = GPIO_Pin_6;//PC7先作为输入，升级时候作为输出
	...
	if(begin_or_end == 0)
	{
		HAL_GPIO_WritePin(GPIOC,GPIO_Pin_6,GPIO_PIN_RESET); //update mode
	}
	else
	{
		HAL_GPIO_WritePin(GPIOC,GPIO_Pin_6,GPIO_PIN_SET); //boot mode	
		GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH;
		GPIO_InitStructure.Pin = GPIO_Pin_6;//PF2先作为输入，升级时候作为输出
...
{
	esp_upload.retriesPerBaudRate = 9; //9种波特率
