 ...
 * M998	- Stop SD print. (Requires SDSUPPORT)
...
*/
float soft_endstop_min[XYZ],soft_endstop_max[XYZ];
void soft_endstop_min_init()
{
        soft_endstop_min[0]= X_MIN_BED;
        soft_endstop_min[1]= X_MIN_BED;
        soft_endstop_min[2]= Z_MIN_POS;

        soft_endstop_max[0] = X_MAX_BED;
        soft_endstop_max[1] = Y_MAX_BED;
...
static void set_axis_is_at_home(const AxisEnum axis) {
  ...
  #endif
  {
    ...
    current_position[C_AXIS] = home_offset[C_AXIS];   //mks_dlp 
  }
...
void do_blocking_move_to_nowait(const float &rx, const float &ry, const float &rz, const float &fr_mm_s/*=0.0*/) {
  const float old_feedrate_mm_s = feedrate_mm_s;
...
void do_blocking_move_to_z_nowait(const float &rz, const float &fr_mm_s/*=0.0*/) {
  do_blocking_move_to_nowait(current_position[X_AXIS], current_position[Y_AXIS], rz, fr_mm_s);
}
...
static void clean_up_after_endstop_or_probe_move() {
  ...
  bool axis_unhomed_error(const bool z/*=true*/) {
    #if ENABLED(HOME_AFTER_DEACTIVATE)
      const bool zz = z && !axis_known_position[Z_AXIS];
    #else
      const bool zz = z && !axis_homed[Z_AXIS];
    #endif
    if (zz) {
      SERIAL_ECHO_START();
...
static void homeaxis(const AxisEnum axis) {
  ...
  #endif
  mksdlp.quick_stop_ena = false;
  do_homing_move(axis, 1.5 * max_length(axis) * axis_home_dir);
  
  mksdlp.buzzer_tone(500,100,2);
      ...
      case MeshNext:
        ...
        if (mbl_probe_index < GRID_MAX_POINTS) {
          ...
          set_bed_leveling_enabled(true);
            ...
            set_destination_from_current();
            buffer_line_to_destination(homing_feedrate_mm_s[Z_AXIS]);
            stepper.synchronize();
          #endif
  ...
  inline void gcode_G38(bool is_38_2) {
    ...
    LOOP_XYZ(i)
      if (FABS(destination[i] - current_position[i]) >= G38_MINIMUM_MOVE) {
        if (!parser.seenval('F')) feedrate_mm_s = homing_feedrate_mm_s[(AxisEnum)i];
...
inline void gcode_M109() {
  ...
  do {
    ...
    if (ELAPSED(now, next_temp_ms)) { //Print temp & remaining time every 1s while waiting
      ...
      //thermalManager.print_heaterstates();    //mks 2018-08
      #if TEMP_RESIDENCY_TIME > 0
...
void quickstop_stepper() {
  ...
  inline void gcode_M428() {
    ...
    LOOP_XYZ(i) {
      ...
      diff[C_AXIS] = home_offset[C_AXIS] - current_position[C_AXIS];    //mks_dlp
      
      if (!WITHIN(diff[i], -200, 200) && home_dir((AxisEnum)i) > 0)
        diff[i] = -current_position[i];
      if (!WITHIN(diff[i], -200, 200)) {
        SERIAL_ERROR_START();
  ...
  switch (parser.command_letter) {
    case 'G': switch (parser.codenum) {
      ...
      case 28: // G28: Home all axes, one at a time
        #if 1
      	if (gCfgItems.breakpoint_reprint_flg == 1) break;
        #endif
        gcode_G28(false);
        break;
    ...
    case 'M': switch (parser.codenum) {
      ...
      case 109: // M109: Wait for hotend temperature to reach target
		...
		//mksdlp.buzzer_tone(1000,1000,5);
		mksdlp.bank2disp_enable(CLEAN_USED_BANK,0,0);
          gcode_M106();
          break;
    ...
    while (--segments) {
      ...
      ADJUST_DELTA(raw); // Adjust Z if bed leveling is enabled
        ...
        oldA = delta[A_AXIS]; oldB = delta[B_AXIS];
          ...
          {
              if (rtarget[Z_AXIS] < gCfgItems.breakpoint_z_pos) 
              {
                  current_position[Z_AXIS] = rtarget[Z_AXIS];
                  current_position[E_AXIS] = destination[E_AXIS];
                  return;
              }
          }
